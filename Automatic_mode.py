...
from RC_control import Automatic
import os
...
print current_time, "Modul autonom a fost activat"
rc.scrie(current_time, " Modul autonom a fost activat\n\n")
...
while True:
    ...
    for (x, y, w, h) in stops:
        ...
        print "Distanta pana la semn: ", distance, " cm"
        rc.scrie(current_time, " Distanta pana la semn: %.2f" %distance + " cm\n")
        if(stop_flag):
            rc.move(1500)
            now = time.time()
            time.sleep(3)
            sec = time.time() - now
            current_time = time.strftime("<%d/%b/%Y-%H:%M:%S>")
            rc.scrie(current_time, " Am stat %d " %sec + " secunde la STOP!\n\n")
            print("Am stat %d" %sec + " secunde la STOP!\n")
            rc.move(1600)
            stop_flag = False
    ...
    for (x, y, w, h) in crosses:
        ...
        if(cross_flag):
            ...
            sec = time.time() - now
            current_time = time.strftime("<%d/%b/%Y-%H:%M:%S>")
            rc.scrie(current_time, " Am stat %d " %sec + " secunde la TRECEREA DE PIETONI!\n\n")
            print("Am stat %d" %sec + " secunde la TRECEREA DE PIETONI!\n")
            rc.move(1600)
            cross_flag = False
    ...
    for (x, y, w, h) in parks:
        ...
        if(park_flag):
            rc.move(1500)
            rc.scrie(current_time, " Am oprit la semnul de PARCARE\n\n")
            print("Am oprit la semnul de PARCARE\n")
            park_flag = False

    if(park_flag == False):
        break

    if(stop_flag and park_flag and cross_flag) == False:
        stop_flag = True
        park_flag = True
        cross_flag = True
